...
NetEdges <- read.csv('NetEdges.csv')
devStages <- read.csv('devStages.csv')
...
ui <- fluidPage(
  ...
  headerPanel( title=div(img(src="./images/logo.svg", width = 60), "SINCE AraBOXcis")),
 # tags$img(src = "./images/logo.svg", width = "99px"),
 h2("G-Box regulatory network for a single cell developmental axis"),
  sidebarPanel(
    ...
    checkboxGroupInput(inputId = "cell", label="Developmental Stage", selected = devStagesInOrder,
      choices = devStagesInOrder)
    # 
    # hr(style = "border-top: 1px solid #000000;"),
    ...
    #             label = "devStages-seq filtering threshold",
    #             min = 0.0,
    #             max = 2,
    #             step=0.01,
    #             value = 0.05),
  ...
  mainPanel(
    tabsetPanel(
      ...
      tabPanel("Overlap Between Stages",
               h3("Heatmap of overlap between developmental stages"),
               p("Rows and columns correspond to developmental stages. Darker green shades refer to more similar/more overlapping networks."),
               p("Please note that each network was scaled down to the top ~50,000 top edges."),
               plotOutput("simpleHeatmap")),
      tabPanel("PIF's",
               h3("Importance of Phytochrome Interacting Factors (PIF's) across development."),
               p("Along the x-axis are the names of the G-Box binding PIF's found in each network. The y axis refers to the \
               number of CHIP-Seq- validated PIF edges in each network."),
               plotOutput("heatmapAnnotCellTypes")),
       tabPanel("Plot IGraph",
                plotOutput("igraphplot")
       ),
       tabPanel("Heatmaps",
              h3("Associations between functional terms within networks"),
     ...
     tabPanel("Citations",
              ...
              p("Leaf devStages-seq: Procko et al. (2022). DOI: 10.1093/plcell/koac167"),
              h3("More info"),
...
server <- function(input, output,session) {
 ...
 # }) %>% debounce(1000)
  ...
  filter_network <- reactive({
    print("We reached our new code")
    print(input$cell)
    print(colnames(devStages))
   ticked_stages <-  devStages[,input$cell]
   if(length(input$cell)>1){
     RowSums <- rowSums(ticked_stages)
     RowSums <- RowSums == length(input$cell) #converts to a Boolean vector, F if 0
     edgesInNets <- which(RowSums) #Finds locations of T rows
  
   NetEdges[edgesInNets,]
   
   } else{
     if(length(input$cell)==1){
       edgesInNets <- which(ticked_stages)
       NetEdges[edgesInNets,]
     }
     else{
       emptyMatrix <- matrix(nrow = 0, ncol = 2)
     }
   }
    
    # if(input$bulkThreshold==0){
    #   edgesToInclude1=c(1:dim(NetEdges)[1])
    #   edgesToInclude2=c(1:dim(NetEdges)[1])}else{
    ...
    #     
    #     edgesToInclude1=which( (NetEdges[,1] %in% genesInRange) & (NetEdges[,2] %in% genesInRange) )
    ...
    #     genesInRange=names(which(maxExpInRange>input$bulkThreshold))
    #     edgesToInclude2=which( (NetEdges[,1] %in% genesInRange) & (NetEdges[,2] %in% genesInRange) )
    ...
    #   
    #   #if(length(cell)>0){
    #   cellsToInclude=devStages[,input$cell]
    #   maxExpInRange=apply(cellsToInclude, 1, max)
    #   genesInRange=names(which(maxExpInRange>input$scThreshold))
    #   edgesToInclude3=which( (NetEdges[,1] %in% genesInRange) & (NetEdges[,2] %in% genesInRange) )
    #  
    #   }else{
    #     
    #     if(length(input$cell)==1){
    #      
    #       maxExpInRange=devStages[,input$cell]
    #    
    #       genesInRange=names(which(maxExpInRange>input$scThreshold))
    #      
    #       edgesToInclude3=which( (NetEdges[,1] %in% genesInRange) & (NetEdges[,2] %in% genesInRange) )
    ...
    # currentNetEdges=NetEdges[edgesToInclude,]
    # NetEdges[edgesToInclude,]
  }) %>% debounce(1000)
  ...
  output$heatmapAnnotCellTypes <- renderPlot({
    temp=mainHeatmap()
    temp=temp[which(rownames(temp) %in% rownames(devStages)),]
    
    if(length(input$cell)==1){
      annotRow=matrix(devStages[rownames(temp), input$cell], byrow=T)
    }else{
    
    annotRow=devStages[rownames(temp), input$cell]
    }
    
    #annotRow=devStages[rownames(temp), cell]
  ...
  output$downloadPathcsv <- downloadHandler(
    filename = function() {
      paste("NetEdgesfiltered-csv-age", input$age[1], "-", input$age[2], "_time", input$time[1], "-", input$time[2], ".csv", sep = "")
    },
  ...
  output$igraphplot <- renderPlot({
    ...
    graph=graph[which(graph[,2] %in% NetEdges[,1]),]
